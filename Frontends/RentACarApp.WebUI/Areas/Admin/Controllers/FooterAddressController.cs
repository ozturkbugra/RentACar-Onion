...
namespace RentACarApp.WebUI.Areas.Admin.Controllers
{
    ...
    public class FooterAddressController : Controller
    {
        ...
        public async Task<IActionResult> Create(CreateFooterAddressDto createFooterAddressDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7066/api/FooterAddresses", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync("https://localhost:7066/api/FooterAddresses/" + id);
            return RedirectToAction("Index", "FooterAddress", new { area = "Admin" });
        ...
        public async Task<IActionResult> Update(int id)
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7066/api/FooterAddresses/" + id);
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        [HttpPost]
        [Route("Update/{id}")]
        public async Task<IActionResult> Update(ResultFooterAddressDto resultFooterAddressDto)
        {
            var client = _httpClientFactory.CreateClient();
            var JsonData = JsonConvert.SerializeObject(resultFooterAddressDto);
            StringContent stringContent = new StringContent(JsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7066/api/FooterAddresses", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "FooterAddress", new { area = "Admin" });
            }
            return View(resultFooterAddressDto);
        }
    }
}
